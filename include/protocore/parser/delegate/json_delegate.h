#ifndef PROTOCORE_JSON_DELEGATE_H
#define PROTOCORE_JSON_DELEGATE_H

#include "../parser.h"
#include "../../3rdpart/jsoncpp/json/value.h"
...
class JsonDelegate;
typedef std::shared_ptr<JsonDelegate> JsonDelegatePtr;

class JsonDelegate : public ValueDelegate
{
public:
    explicit JsonDelegate(const Json::Value &value, const std::string &key = std::string());
    ~JsonDelegate() J_OVERRIDE;
...
public:
    ...
    static ValueDelegatePtr staticCreate(const Json::Value &value,
...
private:
    J_DISABLE_COPY(JsonDelegate)
    Value owner_;
    Json::Value value_;
    Json::Value::const_iterator citer_;
    std::string key_;
};

}   // end of namespace J

#endif  // PROTOCORE_JSON_DELEGATE_H
