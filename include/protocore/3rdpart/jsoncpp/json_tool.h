...
std::wstring JSON_API ansi2Unicode(const std::string& sAnsi);
std::string JSON_API unicode2Ansi(const std::wstring &sUnicode);
std::wstring JSON_API utf82Unicode(const std::string &sUtf8);
std::string JSON_API unicode2Utf8(const std::wstring& sUnicode);
std::string JSON_API utf82Ansi(const std::string &sUtf8);
std::string JSON_API ansi2Utf8(const std::string &sAnsi);

std::string JSON_API dylibSuffix();
int JSON_API createPath(const std::string &path);
bool JSON_API pathExists(const std::string &path);
std::string JSON_API pathOfFile(const std::string &filePath);
std::string JSON_API appDirPath();
...
#if defined(QT_CORE_LIB) && !defined(J_NO_QT)
inline bool qResolve(const QString &filePath, Value &root)
{
    QFile file(filePath);
    ...
    return parse(file.readAll().constData(), root);
}

inline Value qResolve(const QString &filePath, const std::string &path)
{
    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly)) {
        return Value();
    ...
    return parse(file.readAll().constData(), path);
}

inline bool qResolve(const QString &filePath, const std::string &path, Value &value)
{
    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly)) {
        return false;
    ...
    return parse(file.readAll().constData(), path, value);
}
#endif
