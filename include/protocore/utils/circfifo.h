#ifndef PROTOCORE_CIRCFIFO_H
#define PROTOCORE_CIRCFIFO_H
...
template <typename atom, size_t _Count>
class PROTOCORE_EXPORT circfifo
{
    enum : size_t { _COUNT = _Count };   // helper for expression evaluator
public:
    circfifo() : _count(_COUNT), _size(0), _indexRead(0), _indexWrite(0), _buffer(0)
    {
        assert(_count > 0);
    ...
    inline long read(atom* buffer, long count)
    {
        if (_size == 0) {
            ...
            const long overflow = _indexRead + count - _count;
    ...
    {
        long overflow = _size + count - _count;
