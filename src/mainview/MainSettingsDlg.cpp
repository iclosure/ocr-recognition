...
#include "jwt/widget/JTreeView.h"
#include "global.h"
#include <QLayout>
#include <QHeaderView>
#include <QScrollArea>
...
{
    Jwt::saveWidgetState(d_splitter);
}
...
bool SettingsDlg::init()
{
    ...
    if (result) {
        Jwt::restoreWidgetState(d_splitter);
    }
...
void SettingsDlg::onCurrentItemChanged(QStandardItem *current, QStandardItem *previous)
{
    ...
    const QVariant varDomain = current->data(Qt::UserRole + 1);
